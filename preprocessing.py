...
def get_mel(file_path, sr=sr, n_mels=128, n_fft=n_fft, hop_length=hop_length, mono=True):
    audio, sr = librosa.load(file_path, sr=sr, mono=mono)
    audio = audio[:640000]
    
    if(mono == True):
        mel_matrix = librosa.feature.melspectrogram(y=audio, sr=sr, n_mels=n_mels, n_fft=n_fft , hop_length=hop_length)
        
        avg = mel_matrix.mean()
        stdv = mel_matrix.std()
        mel_matrix = (mel_matrix - avg) / stdv
        return mel_matrix
    elif(mono == False):
        mel_L = librosa.feature.melspectrogram(y=audio[0], sr=sr, n_mels=n_mels, n_fft=n_fft , hop_length=hop_length)
        mel_R = librosa.feature.melspectrogram(y=audio[1], sr=sr, n_mels=n_mels, n_fft=n_fft , hop_length=hop_length)
        
        avg = mel_L.mean()
        stdv = mel_L.std()
        mel_L = (mel_L - avg) / stdv
        mel_R = (mel_R - avg) / stdv
        return mel_L, mel_R
...
def generatio_tensor_instances(array_2d, seq_len=100, hop=10, label=None):
...
def preprocessing(data_path, method, sr=sr, n_fft=n_fft, hop_length=hop_length, n_mels=40, n_mfcc=128, exp_b=0.3, seq_len=100, hop=10, mono=True, label=None):
    if label is not None:
        if(mono == True):
            if(method == 'stft'):
                ...
                #mel = get_mel(data_path, sr, n_mels, n_fft, hop_length, mono)
            ...
            if(method == 'stft'):
                ...
                #mel_L, mel_R = get_mel(data_path, sr, n_mels, n_fft, hop_length, mono)
...
def convert_spectrograms(data_dirs, conv_dim, method='mfcc', sr=sr, n_fft=n_fft, hop_length=hop_length, n_mels=40, n_mfcc=128, exp_b=0.3, seq_len=100, hop=10, mono=True, labels=None):
    concat_tensors = []
    ...
    for i, data_dir in tqdm(enumerate(data_dirs)):
        try:
            if labels is not None:
                if(conv_dim == '1d'):
                    concat_tensor, label = preprocessing(data_dir, method=method, sr=sr, n_mels=n_mels, n_mfcc=40, label=labels[i])
                elif(conv_dim == '2d'):
                ...
                if(conv_dim == '1d'):
                    concat_tensor = preprocessing(data_dir, method=method, sr=sr, n_mels=n_mels, n_mfcc=40)
                elif(conv_dim == '2d'):
